...
def data_blurring(data_sample, us_size):
    ...
    return paddle.to_tensor(x_array_blur)
...
def data_preprocessing(target, Image_Size):
    ...
    img = paddle.zeros(shape=[target.shape[0], target.shape[1], Image_Size,
        Image_Size]) # size: [32, 3, 64, 64]
    for idx in range(target.size(0)):
        x = target[idx]
        x = paddle.transpose(x=x, perm=[1, 2, 0]) # x size: [128, 128, 3]
        x = data_blurring(x, Image_Size) # x size: [64, 64, 3]
        img[idx] = paddle.transpose(x=x, perm=[2, 0, 1])

    down_scale_transform = transform = transforms.Resize([Image_Size])
...
class FNO_Dataset(Dataset):
    def __init__(self, data_dir):
        self.data = paddle.load(data_dir)
        self.data = paddle.transpose(self.data, perm=[0,3,1,2])
...
class KMFlowTensorDataset(Dataset):
    def __init__(self, data_path,
        ...
        if stat_path is not None:
            self.stat_path = stat_path
            self.stat = np.load(stat_path).item()  # 使用.item()处理npz文件
        else:
            self.stat = {}
            self.prepare_data()
    ...
    def preprocess_data(self, data):
        ...
        return paddle.to_tensor(data.astype(np.float32))
