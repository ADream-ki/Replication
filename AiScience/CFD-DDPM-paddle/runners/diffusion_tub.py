...
class Diffusion(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                _model = model if model._layers.cond_channels == 0 else lambda x, t: model(x, t, y)
                loss = loss_registry[config.model.type](_model, x, t, e, b)
    ...
    def sample_fid(self, model):
        ...
        if model._layers.cond_channels > 0:
            dataset, test_dataset = get_dataset(args, config)
                ...
                if model._layers.cond_channels > 0:
                    ...
                    if len(y.shape) == 4:
                        if y.shape[1] == 1:
                            ...
                            _y = y
                        for i in range(n):
                            Image.fromarray(np.uint8(_y[i].numpy().transpose([1,2,0])*255)).save(
                                os.path.join(self.args.image_folder, f"{img_id+i}.cond.png")
                            )
                else:
                    _model = model

                x = self.sample_image(x, _model)
    ...
    def sample_sequence(self, model):
        ...
        if model._layers.cond_channels > 0:
            ...
            if len(y.shape) == 4:
                ...
                for i in range(8):
                    Image.fromarray(np.uint8(_y[i].numpy().transpose([1,2,0])*255)).save(
                        os.path.join(self.args.image_folder, f"{i}.cond.png")
                    )
        else:
            _model = model
        ...
        with paddle.no_grad():
            _, x = self.sample_image(x, _model, last=False)

        x = [inverse_data_transform(config, dx) for dx in x]
    ...
    def sample_interpolation(self, model):
        ...
        if model._layers.cond_channels > 0:
            ...
            if len(y.shape) == 4:
                if y.shape[1] == 1:
                    ...
                    _y = y
                Image.fromarray(np.uint8(_y[0].numpy().transpose([1,2,0])*255)).save(
                    os.path.join(self.args.image_folder, f"cond.png")
                )
        else:
            _model = model
        ...
        with paddle.no_grad():
            for i in range(0, x.shape[0], 8):
                xs.append(self.sample_image(x[i : i + 8], _model))
