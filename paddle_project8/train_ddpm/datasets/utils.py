...
def gen_bar_updater():
>>>>>>    pbar = torch.utils.model_zoo.tqdm(total=None)
...
def data_preprocessing(target, Image_Size):
    img = paddle.zeros(shape=[target.shape[0], target.shape[1], Image_Size,
        Image_Size])
    for idx in range(target.shape[0]):
        x = target[idx]
        x = paddle.transpose(x=x, perm=[1, 2, 0])
        x = data_blurring(x, Image_Size)
        img[idx] = paddle.transpose(x=x, perm=[2, 0, 1])
>>>>>>    down_scale_transform = torchvision.transforms.Resize(Image_Size)
    output_target = down_scale_transform(target)
    return img, output_target


class FNO_Dataset(paddle.io.Dataset):

    def __init__(self, data_dir):
        self.data = paddle.load(path=data_dir)
        self.data = paddle.transpose(x=self.data, perm=[0, 3, 1, 2])
...
class KMFlowDataset(paddle.io.Dataset):
    ...
    def prepare_data(self):
        self.scaler = StandardScaler()
>>>>>>        for data_dir in torch.utils.model_zoo.tqdm(self.fname_lst):
            for i in range(self.outer_steps):
                for j in range(0, self.inner_steps, 4):
                    ...
                    del data
        print(
            f'Data statistics, mean: {self.scaler.mean_}, standard deviation: {self.scaler.scale_}'
            )

    def preprocess_data(self, data):
        s = tuple(data.shape)[0]
        sub = int(s // self.resolution)
        data = data[::sub, ::sub]
        data = self.scaler.transform(data.reshape(-1, 1)).reshape((self.
...
class KMFlowTensorDataset(paddle.io.Dataset):
    ...
    def __init__(self, data_path, train_ratio=0.9, test=False, stat_path=
        ...
        print('Data set shape: ', tuple(self.all_data.shape))
        idxs = np.arange(tuple(self.all_data.shape)[0])
        ...
        self.time_step_lst = np.arange(tuple(self.all_data.shape)[1] - 2)
        if not test:
